...
void DrawLine(HDC hdc, int x1, int y1, int x2, int y2)
{
	...
	// 디버그용 그리드 선 초록색으로 설정 
	_newHpen = CreatePen(PS_SOLID, 1, RGB(50, 205, 100));
...
void DrawLineToRectangle(HDC hdc, int left, int top, int right, int bottom)
{
	...
	// 디버그용 그리드 선 초록색으로 설정 
	newHpen = CreatePen(PS_SOLID, 2, RGB(0, 255, 0));
	oldHpen = (HPEN)::SelectObject(g_backBuffer->GetMemDC(), (HGDIOBJ)newHpen);

	DrawLine(g_backBuffer->GetMemDC(), left, top, right, top);
	DrawLine(g_backBuffer->GetMemDC(), right, top, right, bottom);
	DrawLine(g_backBuffer->GetMemDC(), right, bottom, left, bottom);
	DrawLine(g_backBuffer->GetMemDC(), left, bottom, left, top);
