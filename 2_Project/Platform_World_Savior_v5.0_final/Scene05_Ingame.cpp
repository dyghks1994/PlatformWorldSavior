	...
	for (int y = 0; y < MAPSIZE_Y; y++)
	{
		for (int x = 0; x < MAPSIZE_X; x++)
		{
			if (m_pMap->GetType(x, y) == Block::CHAR_START_POS)
			{
				...
				_startPosY = (y * TILESIZE) - TILESIZE; // y좌표는 한칸 보전해줘야 한다.
...
void Scene05_Ingame::ResetCharPos()
{
	...
	for (int y = 0; y < MAPSIZE_Y; y++)
	{
		for (int x = 0; x < MAPSIZE_X; x++)
		{
			...
			if (g_stageNum == 1)
			{
				//// 스테이지 01의 데이터에서 열쇠를 찾아서 시작 위치를 찾음
				...
				//}
				//
				//// 스테이지 01의 데이터에서 닫힌 문의 위치를 찾아서 세팅한다
				...
				//	_doorStartX = x;
				//	_doorStartY = y;
				//	cout << "첫 도어 그리드 X = " << _doorStartX << " Y= " << _doorStartY << endl;
				//
				//	break;
				//}
			}
...
{
	int PlayerGridX = (int)(_player->m_PosX + TILESIZE / 2)/TILESIZE;				// 중심 좌표
...
{
	int PosGridX = _player->m_PosX + TILESIZE / 2;				// 중심 좌표
	...
	wsprintf(current_playerJumpPower, TEXT("캐릭터 JumpPower = %d"), static_cast<int>(_player->JumpPower));
	...
	TextOut(g_backBuffer->GetMemDC(), 10 + _cameraX, 190 + _cameraY, L"스프링 점프력 조절 [T G]", lstrlen(L"스프링 점프력 조절 [T G]"));
	TextOut(g_backBuffer->GetMemDC(), 10 + _cameraX, 210 + _cameraY, L"방향 전환 [컨트롤Ctrl]", lstrlen(L"방향 전환 [컨트롤Ctrl]"));
	TextOut(g_backBuffer->GetMemDC(), 10 + _cameraX, 230 + _cameraY, L"스테이지 리셋 [R]", lstrlen(L"스테이지 리셋 [R]"));
...
{
	bool isDrawDoor = false;	// fals면 문을 그림 2 * 3 , true면 안 그림
